...
def main():
    ...
    if (args.modelFile != "" ):
        ...
        # save the model
        with open('./lidar_model.json', 'w') as outfile:
            json.dump(model.to_json(), outfile)

	# plot the graph of the model
	#plot(model, to_file='./bc_model.png', show_shapes=True)
    ...
    checkpointer = ModelCheckpoint(filepath="./lidar_weights.{epoch:02d}-{loss:.4f}.hdf5", verbose=1, save_weights_only=True)
    ...
    model.fit_generator(generator = data_generator(obs_centroid, obs_size, pickle_dir_and_prefix, 
        ...
        nb_epoch=10, callbacks=[checkpointer,tensorboard])
    print("stop time:")
    print(datetime.datetime.now())
    
    # save model weights
    model.save_weights("./lidar_model.h5", True)
