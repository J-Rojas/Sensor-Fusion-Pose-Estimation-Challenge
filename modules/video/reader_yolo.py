...
Clone: https://github.com/yjxiong/darknet
Follow instructions there for building.

Setup a python site package like: pyDarknet -> /path/to/darknet/pyDarknet

Add a yolo.weights file to /path/to/here/yolo

Then this reader should work.

"""

from pyDarknet.detector import Darknet_ObjectDetector as ObjectDetector
from pyDarknet.detector import DetBBox
...
if __name__ == "__main__":
    ...
    #yolo detector
    det = ObjectDetector('yolo/yolo.cfg','yolo/yolo.weights')
        ...
        if startsec == 0:
            ...
            if t.to_sec() > skipping:
                ...
                if msgType in ['sensor_msgs/Image']:
                    ...
                    # to PIL image and yolo detect
                    pitch = -(img.width * len('RGB'))
                    data = img.get_data('RGB', pitch)
                    pimg = Image.fromstring('RGB', (img.width, img.height), data)
                    rst, run_time = det.detect_object(pimg)

                    print('got %d objects in %f seconds' % (len(rst), run_time))
                    for bbox in rst:
